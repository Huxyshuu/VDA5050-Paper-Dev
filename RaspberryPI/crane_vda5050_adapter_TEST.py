...
VDA 5050 ⇄ OPC UA adapter for Ilmatar overhead crane, with rich logging.
...
Keep the same behavior: XY moves first, optional Z-down, then node actions
(typical sequence: lowerHoist → buttonPress → raiseHoist to 'zu').
...
from new_crane import Crane  # local project import – ensure PYTHONPATH is set
...
TOPIC_ROOT = os.getenv("VDA_TOPIC_ROOT", "uagv/v2/konecranes/ilmatar_1")
MANUFACTURER = os.getenv("VDA_MANUFACTURER", "konecranes")
SERIAL_NUMBER = os.getenv("VDA_SERIAL_NUMBER", "ilmatar_1")
#INTERFACE_VERSION = os.getenv("VDA_INTERFACE_VERSION", "v2")
#INTERFACE_VERSION = "v2"          # major only for topic levels
PROTOCOL_VERSION = "2.1.0"        # full semantic version for message header
LOG_LEVEL = os.getenv("LOG_LEVEL", "INFO").upper()

# Schema directory resolution: ENV→local ./schemas→/mnt/data
ENV_SCHEMA_DIR = os.getenv("VDA_SCHEMA_DIR")
LOCAL_SCHEMA_DIR = Path(__file__).with_name("schemas")
ALT_SCHEMA_DIR = Path("/home/ilmatarlogs/masters_thesis/vda_5050/vda5050_spec/json_schemas")
...
def wait_for_panel_button(btn: str, log: logging.Logger,
    ...
    def _extract(data: dict, button: str) -> Tuple[bool, int]:
        ...
        if "seq" in data and "pressed" in data:
            try: return bool(data["pressed"]), int(data["seq"])
        ...
        if isinstance(sub, dict) and "seq" in sub and "pressed" in sub:
            try: return bool(sub["pressed"]), int(sub["seq"])
            except Exception: return False, 0
        # unknown shape
        return False, 0

    log.info("Waiting for panel button '%s' at %s (timeout=%s) ...", btn, BUTTON_STATUS_URL, timeout)
    t0 = time.time()
    last_heartbeat = 0.0
    ...
    try:
        ...
        if baseline == 0 and isinstance(data0, dict) and btn not in data0:
            log.debug("Panel status baseline shape=%s keys=%s", type(data0).__name__, list(data0.keys())[:6])
    except Exception:
        baseline = 0
        ...
        try:
            ...
            if r.ok:
                ...
                if pressed and seq > baseline:
                    ...
                    consumed = False
                    try:
                        rc = requests.get(BUTTON_STATUS_URL, params={"btn": btn, "consume": 1}, timeout=1.5)
                        if rc.ok: consumed = True
                    except Exception:
                        pass
                    if not consumed:
                        try:
                            requests.get(BUTTON_STATUS_URL, params={"consume": 1}, timeout=1.5)
                        except Exception:
                            pass
                    log.info("Panel button '%s' detected (seq=%d > baseline=%d).", btn, seq, baseline)
                    return True
        except requests.RequestException as e:
            log.debug("Panel poll error for '%s': %s", btn, e)

        if timeout is not None and (time.time() - t0) >= timeout:
            log.warning("Panel wait for '%s' timed out after %.1fs.", btn, time.time() - t0)
            return False
...
def _move_xy_to(crane: Crane, log: logging.Logger, bx_target: int, ty_target: int,
    ...
    while True:
        ...
        if not done_b:
            if crane.move_bridge_to_target_p():
                done_b = True
                try: crane.stop_bridge()
                except Exception: pass

        if not done_t:
            if crane.move_trolley_to_target_p():
                done_t = True
                try: crane.stop_trolley()
                except Exception: pass
        ...
        if now - last_pos_sample > 0.5:
            try:
                ...
                # progress
                if prev_b is None or abs(cur_b - prev_b) >= MOVE_EPS_MM or abs(cur_t - prev_t) >= MOVE_EPS_MM:
                    last_progress = now
                prev_b, prev_t = cur_b, cur_t

                rem_b = abs(bx_target - cur_b)
                rem_t = abs(ty_target - cur_t)
                log.debug("...homing XY: bridge=%d → %d (rem=%d), trolley=%d → %d (rem=%d)",
                          cur_b, bx_target, rem_b, cur_t, ty_target, rem_t)

                if (now - last_warn > 6.0) and (now - last_progress > 6.0) and \
                   (rem_b > NEAR_EPS_MM or rem_t > NEAR_EPS_MM):
...
def _move_hoist_to(crane: Crane, log: logging.Logger, hz_target: int, timeout_s: float = 300.0) -> bool:
    ...
    while True:
        ...
        if now - last_pos_sample > 0.5:
            try:
                ...
                if (now - last_warn > 6.0) and (now - last_progress > 6.0) and (rem > NEAR_EPS_MM):
...
class VDA5050Adapter:
    ...
    def __init__(self, crane: Crane, log: logging.Logger):
        ...
        self.action_states: List[Dict[str, Any]] = []  # lifecycle of current/unfinished actions
        self.operating_mode: str = "AUTOMATIC"  # map your UI/OPC modes as needed
        self.errors: List[Dict[str, Any]] = []  # keep active errors here
        self.battery_state: Dict[str, Any] = {"batteryCharge": 100.0, "charging": False}
        self.safety_state: Dict[str, Any] = {"eStop": "NONE", "fieldViolation": False}
        self.information_messages: List[Dict[str, Any]] = []  # we’ll keep HOIST_POSITION here
        ...
        last_will = self._connection_msg("CONNECTIONBROKEN")
        if not self._validate("connection", last_will): self.log.warning("LWT connection msg failed validation (continuing).")
        self.mqtt.will_set(f"{TOPIC_ROOT}/connection", json.dumps(last_will), qos=1, retain=True)
    ...
    def _on_connect(self, client, userdata, flags, rc):  # noqa: N802
        ...
        client.subscribe([(f"{TOPIC_ROOT}/order", 0), (f"{TOPIC_ROOT}/instantActions", 0)])
        self.log.info("Subscribed to %s/{order,instantActions}", TOPIC_ROOT)
        # announce ONLINE
        online = self._connection_msg("ONLINE")
        if not self._validate("connection", online): self.log.warning("ONLINE connection msg failed validation (continuing).")
        client.publish(f"{TOPIC_ROOT}/connection", json.dumps(online), qos=1, retain=True)
    ...
    def _on_message(self, client, userdata, msg):  # noqa: N802
        ...
        if topic.endswith("/order"):
            if self._validate("order", payload):
                ...
                self.log.info("Order enqueued: orderId=%s, updateId=%s, nodes=%d",
                              payload.get("orderId"), payload.get("orderUpdateId"),
    ...
    def stop(self):
        ...
        try:
            ...
            if not self._validate("connection", offline): self.log.warning("OFFLINE connection msg failed validation (continuing).")
            self.mqtt.publish(f"{TOPIC_ROOT}/connection",
                              json.dumps(offline),
                              qos=1, retain=True)
    ...
    def _start_thread(self, target, name: str):
        th = threading.Thread(target=self._thread_wrapper, args=(target, name), daemon=True, name=name)
        th.start()
    ...
    def _thread_wrapper(self, target, name: str):
        try:
            ...
            self.log.error("Unhandled exception in thread '%s':\n%s", name, traceback.format_exc())
    ...
    def _temporarily_enable_motion_for_reset(self) -> bool:
        ...
        if scale_now is not None and scale_now <= 0.001:
            target = self._speed_scale_backup if (self._speed_scale_backup and self._speed_scale_backup > 0) else 1.0
            if self._try_set_speed_scale(float(target)):
                self.log.debug("Reset: temporarily enabled global speed to %.3f", target)
                return True
        return False
    ...
    def _restore_motion_after_reset_if_needed(self, changed: bool, was_paused: bool):
        ...
        if changed and was_paused:
            ...
            self.log.debug("Reset: restored global speed back to 0 due to paused state.")
    ...
    def _try_get_speed_scale(self) -> Optional[float]:
        for name in ("get_speed_scaling", "get_speed_scale", "get_speed_factor", "get_global_speed"):
    ...
    def _try_set_speed_scale(self, val: float) -> bool:
        for name in ("set_speed_scaling", "set_speed_scale", "set_speed_factor", "set_global_speed"):
            if hasattr(self.crane, name):
                try:
    ...
    def _ramp_speed_scale(self, start: float, end: float, duration_s: float = 1.5, steps: int = 15):
    ...
    def _action_begin(self, action: Dict[str, Any], default_type: str, description: str = "") -> str:
        ...
        desc = action.get("actionDescription") or description or atype
        # replace any previous entry with same id
        self.action_states = [a for a in self.action_states if a.get("actionId") != aid]
    ...
    def _action_update(self, action_id: str, status: Optional[str] = None,
                       result: Optional[str] = None, description: Optional[str] = None):
    ...
    def _action_finish(self, action_id: str, ok: bool = True, result: Optional[str] = None):
        """Mark FINISHED/FAILED and publish."""
        self._action_update(action_id, status=("FINISHED" if ok else "FAILED"), result=result)
    ...
    def _wait_for_button(self, timeout: Optional[float] = None) -> bool:
        ...
        return wait_for_panel_button(BUTTON_NAME_RELEASE, self.log, self._stop_or_cancel, timeout)
    ...
    def _execute_order(self, order_msg: Dict[str, Any]):
        ...
        if edges:
            act_on_edges = sum(1 for e in edges if isinstance(e.get("actions"), list) and len(e["actions"]) > 0)
            self.log.info("Order contains %d edge(s); adapter ignores edge traversal/actions (edge actions=%d).", len(edges), act_on_edges)
        ...
        self.log.info("Begin order execution: orderId=%s updateId=%s nodes=%d", order_id, update_id, len(nodes))
        try:
        ...
        for idx, node in enumerate(nodes):
            ...
            self.log.debug("XY targets: bridge=%d mm, trolley=%d mm", bx_target, ty_target)
            ...
            while not self._stop_or_cancel.is_set():
                ...
                if self._resume_nudge.is_set():
                    ...
                    try:
                        ...
                        if self._active_targets.get("trolley") is not None:
                            self.crane.set_target_trolley(self._active_targets["trolley"])
                        self.log.debug("resume-nudge: reasserted XY targets (bridge=%s, trolley=%s)",
                                        self._active_targets.get("bridge"), self._active_targets.get("trolley"))
                    except Exception:
                        pass
                ...
                if now - start > MAX_XY_S:
                    self.log.error("XY move timeout after %.1fs — stopping motion.", now - start)
                    try:
                        self.crane.stop_all()
                    except Exception: 
                        pass
                ...
                if now - last_log > 0.5:
                    try:
                        cur_b = int(self.crane.get_bridge_position_absolute())
                        cur_t = int(self.crane.get_trolley_position_absolute())
                        self.log.debug("...moving XY: bridge=%d → %d mm, trolley=%d → %d mm",
                                       cur_b, bx_target, cur_t, ty_target)
                        if prev_b is not None and (abs(cur_b - prev_b) > 1 or abs(cur_t - prev_t) > 1):
                            last_progress = now
                        prev_b, prev_t = cur_b, cur_t
                    except Exception:
                        pass
                ...
                if (now - last_progress > 6.0) and not (self._paused or self._hold_mode):
                    self.log.warning("No XY progress — still trying to reach targets.")
                    last_progress = now
                time.sleep(0.1)

            if self._stop_or_cancel.is_set():
                self.log.info("Order canceled/stopped before actions; aborting current order.")
                self._force_hold_pose()
                self._driving = False
                self._publish_state()
            ...
            for a_idx, action in enumerate(actions):
                if not isinstance(action, dict) or "actionType" not in action:
                    self.log.error("Skipping malformed action on node %s: %r", node_id, action)
                    continue
                if self._stop_or_cancel.is_set():
                    self.log.info("Stop/Cancel during actions; aborting current order.")
                    return
                
                self.log.info("-> Action %d/%d: %s", a_idx + 1, len(actions), action.get("actionType"))
                # announce action as WAITING before execution
                aid_wait = self._action_begin(action,
                ...
                if self._stop_or_cancel.is_set():
                    self.log.info("Stop/Cancel after actions; skipping node completion.")
                    return
            ...
            if self.node_states and self.node_states[0].get("nodeId") == self.last_node_id \
        ...
        self.log.info("Order execution finished: orderId=%s updateId=%s", order_id, update_id)
    ...
    def _execute_instant_actions(self, ia_msg: Dict[str, Any]):
        ...
        try:
            if not self._validate("instantActions", ia_msg):
                self.log.error("instantActions failed validation at execute-time; dropping.")
                return
        except Exception:
            pass
        ...
        for action in actions:
            bt = (action.get("blockingType") or "HARD").upper()
            at = action.get("actionType")
            self.log.info("InstantAction: type=%s blockingType=%s", at, bt)
            ...
            # Minimal semantics for blockingType:
            # - HARD: stop all motion first (exclusive)
            # - SOFT: don't move while executing (hold current pose)
            # - NONE: proceed without extra constraints

            if bt == "HARD":
                if self._is_any_motion():
                    try:
                        self.crane.stop_all()
                        self.log.debug("blockingType=HARD: stop_all() issued (motion detected).")
                    except Exception:
                        pass
                else:
                    self.log.debug("blockingType=HARD: no motion; no stop issued.")
            elif bt == "SOFT":
                if self._is_xy_moving():
                    try:
                        self.crane.stop_bridge()
                        self.crane.stop_trolley()
                        self.log.debug("blockingType=SOFT: XY stop issued (driving detected).")
                    except Exception:
                        pass
                else:
                    self.log.debug("blockingType=SOFT: XY already stationary; no stop issued.")
            # NONE -> no pre-stop
    ...
    def _execute_action(self, action: Dict[str, Any]):
        ...
        t0 = time.time()
        self.log.info("[action start] %s", atype)
        ...
        try:
            if atype == "lowerHoist":
                ...
                if zd_m is None:
                    ...
                    self.log.warning("lowerHoist: no zd provided; using fallback %d mm", target_mm)
                else:
                    try:
                        target_mm = int(float(zd_m) * 1000)
                    except (TypeError, ValueError):
                        self.log.warning("lowerHoist: invalid zd=%r; using fallback 445 mm", zd_m)
                        target_mm = 445

                self._active_targets["hoist"] = target_mm
                self.crane.set_target_hoist(target_mm)
                aid = self._action_begin(action, default_type="lowerHoist",
                ...
                while not self._stop_or_cancel.is_set():
                    ...
                    if self._resume_nudge.is_set():
                        ...
                        try:
                            ...
                            self.crane.set_target_hoist(int(hz) if hz is not None else int(target_mm))
                            self.log.debug("resume-nudge: reasserted Z target (%s)", hz if hz is not None else target_mm)
                        except Exception:
                            pass
                        time.sleep(0.1)
                    ...
                    if now - last_pos_log > 0.5:
                        try:
                            ...
                            self.log.debug("... lowering: hoist=%d mm → %d mm (rem=%d mm)", pos, target_mm, rem)
                            self._action_update(aid, result=f"Hoist height: {pos/1000.0:.3f} m (rem={rem/1000.0:.3f} m)")

                            if (now - last_warn > 6.0) and (now - last_progress > 6.0) and not (self._paused or self._hold_mode) and (rem > NEAR_EPS_MM):
                                self.log.warning("No Z progress — still trying to reach target.")
                                last_warn = now
                        except Exception:
                            pass
                        last_pos_log = now
                ...
                try:
                    ...
                    pass
                self._action_finish(aid, ok=True, result=f"Reached {target_mm/1000.0:.3f} m")
                return
                ...
                try:
                    ...
                    to = None
                aid = self._action_begin(action, default_type="buttonPress",
                                         description=f"Waiting for '{BUTTON_NAME_RELEASE}'")
                self._action_update(aid, status="RUNNING")

                btn = BUTTON_NAME_RELEASE
                self.log.info("buttonPress: waiting for '%s' instantAction OR POST from %s (timeout=%s) ...", btn, BUTTON_STATUS_URL, to)
                ...
                while not self._stop_or_cancel.is_set():
                    ...
                    self._drain_instant_actions_nonblocking()
                    if self._stop_or_cancel.is_set():
                        self.log.info("buttonPress: canceled/stopped; exiting wait immediately.")
                        self._action_finish(aid, ok=False, result="canceled")
                        return
                    ...
                    with self._release_lock:
                        if self._release_ctr > baseline_release:
                            self.log.info("buttonPress: saw release instantAction (ctr=%d > %d).",
                                          self._release_ctr, baseline_release)
                            self._action_finish(aid, ok=True, result="release instantAction received")
                            return
                            ...
                            if pressed and seq > baseline:
                                ...
                                if not consumed:
                                    try:
                                        ...
                                        pass
                                self.log.info("buttonPress: detected press (seq=%d > baseline=%d).", seq, baseline)
                                self._action_finish(aid, ok=True, result="panel button pressed")
                                return
                    except requests.RequestException as e:
                    ...
                    now = time.time()
                    if now - last_heartbeat > 2.0:
                        self.log.info("buttonPress: ...still waiting (%.1fs elapsed)", now - t0)
                        last_heartbeat = now
                    time.sleep(0.2)
                ...
                self.crane.set_target_hoist(target_mm)
                self.log.info("raiseHoist: raising to zu=%.3f m (%d mm)", float(zu_m) if zu_m is not None else 3.071, target_mm)

                aid = self._action_begin(action, default_type="raiseHoist",
                ...
                while not self._stop_or_cancel.is_set():
                    ...
                    if now - last_pos_log > 0.5:
                        try:
                            ...
                            prev_pos = pos
                            rem = abs(target_mm - pos)
                            self.log.debug("... raising: hoist=%d mm → %d mm (rem=%d mm)", pos, target_mm, rem)
                            self._action_update(aid, result=f"Hoist height: {pos/1000.0:.3f} m (rem={rem/1000.0:.3f} m)")
                ...
                if scale_now is not None and self._try_set_speed_scale(scale_now):
                    ...
                    self.log.info("pause: ramping speed scale %.3f → 0.0", self._speed_scale_backup)
                    self._ramp_speed_scale(self._speed_scale_backup, 0.0, duration_s=1.5, steps=15)
                    ...
                    self.log.info("pause: entering hold-mode (no speed scale API detected).")
                ...
                if not self._hold_mode:
                    ...
                    start = 0.0
                    end   = self._speed_scale_backup if (self._speed_scale_backup and self._speed_scale_backup > 0) else 1.0
                    ...
                    self.log.info("resume: leaving hold-mode; loops will continue toward active targets.")
                    self._hold_mode = False
                ...
                try:
                    ...
                    self.log.debug("resume: re-applied targets (bridge=%s, trolley=%s, hoist=%s).", bx, ty, hz)
                except Exception as e:
                    self.log.debug("resume: re-applying targets failed (non-fatal): %s", e)
                ...
                self._cancel.set()
                self.log.warning("cancelOrder: cancel flag set. Stopping motion and aborting current order.")
                try:
                    self.crane.stop_all()
                except Exception:
                    pass
                ...
                if self._paused and not self._hold_mode:
                    end_scale = self._speed_scale_backup if (self._speed_scale_backup and self._speed_scale_backup > 0) else 1.0
                ...
                while True:
                    ...
                    try:
                        ...
                        t = ""
                    if t in ("pause", "resume", "cancelOrder"):
                        tmp.append(ia)
                    else:
                        cleared += 1
                ...
                if cleared:
                    self.log.info("cancelOrder: dropped %d pending instant action(s).", cleared)
                ...
                self.log.info("resetBridgeTrolley: homing XY to bridge=%d, trolley=%d ...", HOME_BRIDGE_MM, HOME_TROLLEY_MM)
                _move_xy_to(self.crane, self.log, HOME_BRIDGE_MM, HOME_TROLLEY_MM, timeout_s=300.0)
                try:
                    self.crane.stop_bridge()
                except Exception:
                    pass
        ...
        try:
            ...
            try:
                ...
                info_item = {
                    ...
                    "infoLevel": "INFO",  # required by your schema
                ...
                self.information_messages = [
                    i for i in self.information_messages if i.get("infoType") != "HOIST_POSITION"
                ]
            ...
            payload = {
                "headerId": self.header.next("state"),
                "timestamp": utc_ts(),
                "version": PROTOCOL_VERSION,
                "manufacturer": MANUFACTURER,
                "serialNumber": SERIAL_NUMBER,

                # --- required by your static state schema ---
                ...
                "nodeStates": self.node_states,      # you can fill these later if desired
                "edgeStates": self.edge_states,      # idem
                "driving": self._driving,                    # True while XY axes are moving; hoist-only motion not counted
                "actionStates": self.action_states,                  # keep empty unless you track lifecycles
                "batteryState": self.battery_state,  # {"batteryCharge": ..., "charging": ...}
                "operatingMode": self.operating_mode,  # "AUTOMATIC"/"MANUAL"/etc.
                "errors": self.errors,               # [] if none
                "safetyState": self.safety_state,    # {"eStop": "...", "fieldViolation": ...}

                # --- extras (valid and helpful) ---
                "agvPosition": {
                    "x": bridge_mm / 1000.0,
                    "y": trolley_mm / 1000.0,
                    "theta": 0.0,                 # required by your schema
                    "mapId": "map",
                    "positionInitialized": True   # required by your schema
                },
                "velocity": {
                    "vx": 0.0,
                    "vy": 0.0,
            ...
            try:
                if not self._validate("state", payload):
                    self.log.warning("State payload failed schema validation (continuing).")
            except Exception:
                pass
        ...
        try:
            ...
            try:
                if not self._validate("visualization", payload):
                    self.log.warning("Visualization payload failed schema validation (continuing).")
            except Exception:
                # if visualization schema isn't loaded for some reason, just continue
                pass

            payload_s = json.dumps(payload)
            self.mqtt.publish(f"{TOPIC_ROOT}/visualization", payload_s, qos=0, retain=False)
...
def main():
    ...
    log.info("Config: broker=%s:%s, topic_root=%s, button_url=%s",
             BROKER_HOST, BROKER_PORT, TOPIC_ROOT, BUTTON_STATUS_URL)

    # read access code & URL
    with open("accesscode_url.txt", "r", encoding="utf-8") as fh:
    ...
    watchdog_thread = threading.Thread(
        target=_watchdog_loop, args=(crane, watchdog_stop, log),
    ...
    ok_auto = wait_for_panel_button(BUTTON_NAME_AUTOMATIC, log,
                                    stop_event=shutdown_evt,  # allow Ctrl-C to break the wait
                                    timeout=AUTO_WAIT_S)

    if not ok_auto:
        log.warning("AUTOMATIC not pressed within %.1fs -- skipping Z/XY homing and starting adapter.", AUTO_WAIT_S)
    else:
        ...
        if ok_z:
            ok_xy = _move_xy_to(crane, log, HOME_BRIDGE_MM, HOME_TROLLEY_MM, timeout_s=300.0)
        else:
            ok_xy = False
        ...
        if ok_z and ok_xy:
            ...
            log.error("Homing failed or timed out; staying in STOP, but starting adapter for visibility.")
    ...
    log.info("---------------- | VDA5050 adapter running – Press Ctrl-C to stop | ---------------")
    try:
        while True:
            time.sleep(1)
