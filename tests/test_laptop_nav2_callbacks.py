"""Exercise actual adapter/laptop callback methods without ROS hardware imports."""
import ast
import copy
import json
import threading
...
class Nav2CallbacksTests(unittest.TestCase):
    def setUp(self):
        ...
        a._order={'orderId':'trial-1','orderDescription':'laptop-timing-v1 benchmark'}
    ...
    def test_laptop_timestamps_mqtt_callback_entry_and_ignores_old_order(self):
        cls=methods('benchmark/rox_client.py','RoxRunner',{'_feedback'})
        pi=cls(); pi.probe_id='probe'; pi.ready_event=threading.Event()
        row={'trial_id':'trial-1','pair_id':'pair-1','mode':'vda','measure':'true','device':'rox','start':'A','target':'B'}
    ...
    def test_laptop_native_callbacks_record_same_milestones(self):
        cls=methods('benchmark/rox_client.py','RoxRunner',{'_native_ack','_native_result'})
        pi=cls(); logger=MemoryLogger()
        row={'trial_id':'n','pair_id':'p','mode':'native','measure':'true','device':'rox','start':'A','target':'B'}
        ...
        result.callback(result)
        self.assertEqual(['COMMAND_ISSUED','COMMAND_ACK_RECEIVED','COMMAND_RESULT_RECEIVED'],
                         [e['event_type'] for e in logger.events])

    def test_vda_goal_has_the_same_frozen_target_as_native(self):
        import yaml, jsonschema
        cls=methods('benchmark/rox_client.py','RoxRunner',{'target','vda_order'})
    ...
    def test_ui_uses_the_two_pi_durations_and_preserves_ns_as_text(self):
        ...
        row={'trial_id':'v','pair_id':'p','mode':'vda','measure':'true','device':'rox','start':'A','target':'B'}
