...
ALT_SCHEMA_DIR = Path(
    "/home/ilmatarlogs/masters_thesis/vda_5050/vda5050_spec/json_schemas"
)
...
class VDA5050Adapter:
    ...
    def _on_connect(self, client, userdata, flags, rc):  # noqa: N802
        ...
        client.subscribe(
            [(f"{TOPIC_ROOT}/order", 0), (f"{TOPIC_ROOT}/instantActions", 0)]
        )
            ...
            if bt == "HARD":
                if self._is_any_motion():
                    try:
                        self.crane.stop_all()
                        self.log.debug(
                            "blockingType=HARD: stop_all() issued (motion detected)."
                        )
                    except Exception:
                        pass
                else:
                ...
                if self._is_xy_moving():
                    try:
                        ...
                        self.crane.stop_trolley()
                        self.log.debug(
                            "blockingType=SOFT: XY stop issued (driving detected)."
                        )
                    except Exception:
                        pass
                else:
                    self.log.debug(
                        "blockingType=SOFT: XY already stationary; no stop issued."
                    )
            # NONE -> no pre-stop
...
def main():
    ...
    log.info(
        "Config: broker=%s:%s, topic_root=%s, button_url=%s",
        BROKER_HOST,
        BROKER_PORT,
        TOPIC_ROOT,
        BUTTON_STATUS_URL,
